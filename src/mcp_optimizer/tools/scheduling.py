...
class EmployeeConstraints(BaseModel):
    ...
    max_shifts_per_week: Optional[int] = Field(default=None, ge=0)
    min_shifts_per_week: Optional[int] = Field(default=None, ge=0)
    unavailable_shifts: List[str] = Field(default_factory=list)
    preferred_shifts: List[str] = Field(default_factory=list)
    skills: List[str] = Field(default_factory=list)
    max_consecutive_shifts: Optional[int] = Field(default=None, ge=1)
...
def solve_job_scheduling(input_data: Dict[str, Any]) -> OptimizationResult:
    ...
    try:
        ...
        machine_intervals = {machine_idx: [] for machine_idx in range(len(machines))}
        ...
        for job in jobs:
            for task_idx, task in enumerate(job.tasks):
                ...
                interval_var = model.NewIntervalVar(start_var, duration, end_var, f'interval_{job.id}_{task_idx}')
        ...
        for machine_idx, intervals in machine_intervals.items():
            if intervals:
        ...
        if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
            ...
            for job in jobs:
                ...
                completion_time = max(task["end_time"] for task in job_schedule)
            ...
            return OptimizationResult(
                ...
                objective_value=makespan if scheduling_input.objective == "makespan" else total_completion_time,
                variables={
                    "schedule": schedule,
                    "makespan": makespan,
    ...
    try:
        ...
        for emp_idx, employee in enumerate(employees):
            ...
            if emp_constraints.max_consecutive_shifts is not None:
                for start_day in range(days - emp_constraints.max_consecutive_shifts):
                    consecutive_vars = []
                    for day in range(start_day, start_day + emp_constraints.max_consecutive_shifts + 1):
                        day_working = model.NewBoolVar(f'working_{employee}_{day}')
                        model.Add(day_working == sum(
                            assignments[emp_idx][shift_idx][day]
                            for shift_idx in range(len(shifts))
                        ))
        ...
        if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
            ...
            for day in range(days):
                ...
                for shift_idx, shift in enumerate(shifts):
                    assigned_count = sum(
                        solver.Value(assignments[emp_idx][shift_idx][day])
                        for emp_idx in range(len(employees))
                    )
                    day_coverage.append({
                        "shift_name": shift.name,
                        ...
                        "coverage_ratio": assigned_count / shift.required_staff if shift.required_staff > 0 else 1.0
                    })
            ...
            return OptimizationResult(
                ...
                variables={
                    ...
                    "total_assignments": sum(emp["total_shifts"] for emp in employee_stats.values()),
...
def register_scheduling_tools(mcp: FastMCP[Any]) -> None:
    ...
    def solve_employee_shift_scheduling(
        ...
        days: int,
        employee_constraints: Optional[Dict[str, Dict[str, Any]]] = None,
        time_limit_seconds: float = 30.0
    ) -> Dict[str, Any]:
