...
class TSPInput(BaseModel):
    ...
    def validate_start_location(cls, v, values):
...
class VRPInput(BaseModel):
    ...
    @validator('depot')
    def validate_depot(cls, v, values):
        if 'locations' in values and v >= len(values['locations']):
...
def solve_traveling_salesman(input_data: Dict[str, Any]) -> OptimizationResult:
    ...
    try:
        ...
        if tsp_input.distance_matrix:
            ...
            if len(distance_matrix) != n_locations or \
        ...
        manager = pywrapcp.RoutingIndexManager(
            n_locations, 1, tsp_input.start_location
        )
...
def solve_vehicle_routing(input_data: Dict[str, Any]) -> OptimizationResult:
    ...
    try:
        ...
        # Create routing model
        manager = pywrapcp.RoutingIndexManager(
            n_locations, n_vehicles, vrp_input.depot
        )
        routing = pywrapcp.RoutingModel(manager)
        ...
        if solution:
            ...
            for vehicle_id in range(n_vehicles):
                ...
                if len(route) > 1:  # Only include routes with actual stops
                    routes.append({
                        ...
                        "utilization": route_load / vehicles[vehicle_id].capacity if vehicles[vehicle_id].capacity > 0 else 0
                    })
            ...
            return OptimizationResult(
                ...
                variables={
                    ...
                    "num_locations_served": sum(len(r["route"]) - 1 for r in routes)  # Exclude depot returns
                },
                execution_time=execution_time,
                solver_info={
                    "solver_name": "OR-Tools Routing",
